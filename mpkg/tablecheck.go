...
func Tablecount(srchostIp, srchostPort, srcUser, srcUserPasswd, dsthostIp, dsthostPort, dstUser, dstUserPasswd string, Tablecountsthread int, nocheckdb []string) {
	_, _ = ConnectDB(srchostIp, srchostPort, srcUser, srcUserPasswd)
	GetDB(nocheckdb)
	srctablecout := Process(Tablecountsthread)
	CloseDB()
	_, _ = ConnectDB(dsthostIp, dsthostPort, dstUser, dstUserPasswd)
	GetDB(nocheckdb)
...
func GetDB(nocheck []string) {
	var sql_get_db string
	var dbname []string
	args := "("
	i := 1
	for _, v := range nocheck {
		if i < len(nocheck) {
			args = args + "'" + v + "',"
		} else {
			args = args + "'" + v + "'"
	...
	f := "select SCHEMA_NAME  from information_schema.SCHEMATA where SCHEMA_NAME not in "
	ignoreRegx := args + ")"
	sql_get_db = f + ignoreRegx
	//db, err := DB.Query(sql_get_db, "mysql", "information_schema", "performance_schema", "sys")
	fmt.Println(sql_get_db)
	db, err := DB.Query(sql_get_db)
	if err != nil {
		queryErr := err.Error()
