...
func ConnectDB(ip, port, userName, password string) (dbconn *sql.DB, err error) {
...
func CountDBSize() (err error) {
	...
	for _, v := range dbname {
		...
		sqlText := "SELECT  TABLE_SCHEMA,table_name,ROUND((DATA_LENGTH+INDEX_LENGTH)) as SizeBit,ENGINE" +
			" FROM " +
			" information_schema.tables" +
			" WHERE table_schema= '" +
			v +
			"' AND ENGINE is not NULL " +
			"  ORDER BY SizeBit DESC;"
	...
	// TODO 建议把数据大小转为humansize的方式 这样更合适一点的  --> 完成
	PrintLog("================================ TOTAL SIZE ================================")
	sumsql := "select SUM(ROUND(DATA_LENGTH+INDEX_LENGTH)) AS TOTAL_SIZE_Bit " +
		"FROM " +
		"information_schema.tables " +
		"WHERE " +
		"ENGINE is not null"
