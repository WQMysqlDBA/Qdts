...
func createdstuser56_57() {
	...
	_, _ = mpkg.ConnectDB(DsthostIp, DsthostPort, DstUser, DstUserPasswd)
	mpkg.CreateUser()
	mpkg.CloseDB()
...
func main() {
	...
	if stepstep != "0" {
		...
		storage.UpdatePos(s)
	} else if storage.ReadPos() != "0" {
		/* 如果不加命令行参数执行第几步，!=0，比如=1，表示执行第一步成功了，那么后面应该从第2步执行，所以UpdatePos为s+1 */
		s, _ := strconv.Atoi(storage.ReadPos())
		storage.UpdatePos(s + 1)
	}
	/* 如果还==0，表示没加step参数，默认从1开始 */

	if a, _ := strconv.Atoi(storage.ReadPos()); a >= _step && stepstep == "0" {
		log.Fatal(fmt.Sprintf("offset标识上次的迁移已经全部完成，请先清空%s文件", storage.FlagFilePath))
	}

	if storage.ReadPos() == "0" {
		a, _ := strconv.Atoi(storage.ReadPos())
		log.Println("choose step: ", a+1)
	} else {
		log.Println("choose step: ", storage.ReadPos())
	}
	/* debug */
...
func step1() {
	/* dump data file from src mysql instance */
...
func step4() {
	...
	/* set sql mode */
	setsqlmode()
	/* load data to dst mysql instance */
	Srcconn := make(map[string]string)
	Srcconn["ip"] = CFG.SrchostIp
	Srcconn["port"] = CFG.SrchostPort
	Srcconn["user"] = CFG.SrcUser
	Srcconn["passwd"] = CFG.SrcUserPasswd
	...
	//dstconn, _ := mpkg.ConnectDB(DsthostIp, DsthostPort, DstUser, DstUserPasswd)
	//mpkg.CreateSchema(dstconn,DumpfileDir)
	//mpkg.CloseDB()

	_ = mpkg.MyLoader(Srcconn, DsthostIp, DsthostPort, DstUser, DstUserPasswd, LoadThread, Loadloglevel, DumpfileDir, t, IngoreDB)
	storage.UpdatePos(4)
}
...
func step6(ignoredb []string) {
	// 进行数据统计
	mpkg.PrintLog("#############进行数据行数校验已经支持花里花哨的数据库名称，如`testdb 1` ,'testdbv1.0' etc. ")
	t, err := strconv.Atoi(CFG.TableCheckConfig.Tablecountsthread)
	if err != nil {
		mpkg.Color(102, "* Error")
		log.Println(err)
	}
	mpkg.Tablecount(SrchostIp, SrchostPort, SrcUser, SrcUserPasswd, DsthostIp, DsthostPort, DstUser, DstUserPasswd, t, ignoredb)
	storage.UpdatePos(6)
}
...
`
	...
	fmt.Println(color.Yellow(_UI))
	fmt.Println(color.Green(WelcomeTxt))
	fmt.Println(color.Green(startMsg))
}
