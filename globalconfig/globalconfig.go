...
func checkConfig(c *ConfigInfo) string {
	...
	if c.MydumperConfig.DumpThread == "" {
		...
		mpkg.PrintLog(fmt.Sprintf("Dumpthread is nil,using default value %s", _myDumpthread))
	}
	if c.MyloaderConfig.LoadThread == "" {
		c.MyloaderConfig.LoadThread = _myLoadthread
		mpkg.PrintLog(fmt.Sprintf("Loadthread is nil,using default value %s", _myLoadthread))
	}
	if c.TableCheckConfig.Tablecountsthread == "" {
		...
		mpkg.PrintLog(fmt.Sprintf("数据行数校验并行协程数is nil,using default value %v(cpu num*2)", _tbCountthread))
	...
	if c.TableCheckConfig.Tablechecksumthread == "" {
		...
		mpkg.PrintLog(fmt.Sprintf("数据checksum校验并行协程数is nil,using default value %v(cpu num*2)", _tbCheckthread))
	}
	return ""
}
