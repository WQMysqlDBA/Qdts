...
func Checklicense() {
	...
	f, err := hex.DecodeString(strings.Split(readlicense(lic), "@Wsq12qa1Q&")[0])
	if err != nil {
		mpkg.Color(102, "[** Error]")
		log.Println(err.Error())
	}
	v1, err1 := hex.DecodeString(strings.Split(readlicense(lic), "@Wsq12qa1Q&")[1])
	if err1 != nil {
		mpkg.Color(102, "[** Error]")
	...
	tmp1, err2 := time.ParseInLocation(timeLayout, time1, loc)
	if err2 != nil {
		mpkg.Color(102, "[** Error]")
		log.Println(err2.Error())
	}
	tmp2, err3 := time.ParseInLocation(timeLayout, time2, loc)
	if err3 != nil {
		mpkg.Color(102, "[** Error]")
